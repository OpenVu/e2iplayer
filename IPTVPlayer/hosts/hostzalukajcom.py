...
###################################################
config.plugins.iptvplayer.zalukajtv_filmssort    = ConfigSelection(default = "ostatnio-dodane", choices = [("ostatnio-dodane", "ostatnio dodane"), ("ostatnio-ogladane", "ostatnio oglÄdane"), ("odslon", "odsÅon"), ("ulubione", "ulubione"), ("oceny", "oceny"), ("mobilne", "mobilne")]) 
...
def GetConfigList():
    ...
    if config.plugins.iptvplayer.zalukajtvPREMIUM.value:
        ...
        optionList.append(getConfigListEntry("  " + _("hasÅo") + ":", config.plugins.iptvplayer.zalukajtv_password))
...
class ZalukajCOM(CBaseHostClass):
    ...
    FILMS_SUB_MENU = [{ 'category':'films_category', 'title':'Kategorie',        'url':MAIN_URL },
                      { 'category':'films_list',     'title':'Ostatnio oglÄdane', 'url':MAIN_URL + '/cache/lastseen.html' },
    ...
    FILMS_POPULAR = [{ 'category':'films_list', 'title':'Wczoraj',        'url':MAIN_URL + '/cache/wyswietlenia-wczoraj.html' },
                     ...
                     { 'category':'films_list', 'title':'W tym miesiÄcu', 'url':MAIN_URL + '/cache/wyswietlenia-miesiac.html'} ]
    ...
    LANGS_TAB = [{ 'title':'Wszystkie',     'lang':'wszystkie'      },
                 ...
                 { 'title':'NietÅumaczone', 'lang':'nie-tlumaczone' } ]
    ...
    def getLinksForVideo(self, cItem):
        ...
        for loggedIn in tries:
            ...
            if '' == url:
                ...
                data = self.cm.ph.getDataBeetwenMarkers(data, 'OglÄdaj Film Online', '<div class="doln">', False)[1]
    ...
    def tryTologin(self):
        ...
        if None == self.loggedIn or self.login != config.plugins.iptvplayer.zalukajtv_login.value or\
            ...
            sts, msg = False, 'Problem z zalogowaniem uÅ¼ytkownika \n"%s".' % self.login
            post_data = None
            ...
            if sts:
                ...
                if sts:
                    ...
                    if sts: 
                        tmp = tmp.replace('(kliknij by oglÄdaÄ bez limitÃ³w)', '')
                        msg = 'ZostaÅeÅ poprawnie zalogowany.' + '\nTyp Konta: '+self.cleanHtmlStr(tmp)
                        tmp = self.cm.ph.getDataBeetwenMarkers(data, '<p>Zebrane Punkty:', '</p>', False)[1].replace('&raquo; WymieÅ na VIP &laquo;', '')
